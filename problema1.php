...
if ( $iMax <= $iMin ) {
	echo("iMin debe ser menos a iMax");
	return false;
}


if (!is_int($iMin) || !is_int($iMax)) {
	echo("Los parametros deben ser numeros enteros.");
	return false;
}


if( $iMin < 0 || $iMax > 999 || $iMax < 0 || $iMin > 999 ){
	echo("El valor de los parametros debe ir en un rango de entre 0 y 999.");
	return false;
}

if( !is_array($aDatos) ){
	echo("El parametro debe ser un Array.");
	return false;
}

if( !empty($aDatos) ){
	echo("El Array debe ester vacio.");
	return false;
}
	...
	//Validaciones y creacion del array
